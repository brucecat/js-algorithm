...
function construct(grid) {
    ...
    var preSum = new Array(n + 1).fill(0).map(function () { return new Array(n + 1).fill(0); });
    for (var i = 1; i <= n; i++) {
        for (var j = 1; j <= n; j++) {
    ...
    function dfs(x1, y1, x2, y2) {
        var len = x2 - x1 + 1;
        var sum = getSum(x1, y1, x2, y2);
        if (sum === 0 || sum === len * len)
            return new Node(sum === 0 ? false : true, true); //矩阵值相同
        var m = len / 2;
        // 递归四块
        var res = new Node(false, false, dfs(x1, y1, x1 + m - 1, y1 + m - 1), dfs(x1, y1 + m, x1 + m - 1, y2), dfs(x1 + m, y1, x2, y1 + m - 1), dfs(x1 + m, y1 + m, x2, y2));
        return res;
    }
    return dfs(1, 1, n, n);
}
...
var construct01 = function (grid) {
    var n = grid.length;
    var s = new Array(n + 1).fill(0).map(function () { return new Array(n + 1).fill(0); });
    // 预处理前缀和
    for (var i = 1; i <= n; i++)
        for (var j = 1; j <= n; j++)
    ...
    function dfs(x1, y1, x2, y2) {
        var len = x2 - x1 + 1, sum = getSum(x1, y1, x2, y2);
        if (sum === 0 || sum === len * len)
            return new Node(!!sum, true); //矩阵值相同
        var m = len / 2;
        // 递归四块
        return node = new Node(0, false, dfs(x1, y1, x1 + m - 1, y1 + m - 1), dfs(x1, y1 + m, x1 + m - 1, y2), dfs(x1 + m, y1, x2, y1 + m - 1), dfs(x1 + m, y1 + m, x2, y2));
    }
    return dfs(1, 1, n, n);
};
