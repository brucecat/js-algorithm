...
function findLengthOfShortestSubarray01(arr: number[]): number {
    let len = arr.length;

    // leftEnd作为左边区域 结束位置的指针,rightStart作为右边区域 起始位置的指针
    let leftEnd = 0, rightStart = len - 1;
    ...
    let deleteMinLen = Math.min(len - leftEnd - 1, rightStart);
...
function findLengthOfShortestSubarray02(arr: number[]): number {
    let len = arr.length;
...
// 二份查找的非递归实现版本
function findFirstBigIndex(arr: number[], left: number, num: number) {
    let low = left, hight = arr.length - 1;
    while (low <= hight) {
